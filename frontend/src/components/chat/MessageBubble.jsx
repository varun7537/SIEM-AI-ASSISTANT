  ...
  const renderMetadata = () => {
    ...
    return (
      <div className="mt-6 space-y-4">
        {/* Query Metadata with Enhanced Design */}
        <div className="flex items-center justify-between p-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl border border-slate-200">
          <div className="flex items-center gap-6 text-sm text-slate-600">
            ...
            {confidence && (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg shadow-sm border border-slate-200">
                <div className={`h-2 w-2 rounded-full ${confidence > 0.8 ? 'bg-green-500' : confidence > 0.6 ? 'bg-yellow-500' : 'bg-orange-500'}`}></div>
            ...
            {data.summary.event_types && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {Object.entries(data.summary.event_types).map(([type, count]) => (
                  <div key={type} className="group bg-white p-4 rounded-lg border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-200">
                    ...
                    <div className="w-full h-1 bg-slate-200 rounded-full mt-2 overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transform origin-left group-hover:scale-x-105 transition-transform duration-200" style={{width: '75%'}}></div>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.events.slice(0, 5).map((event, index) => (
                    <tr key={index} className="hover:bg-slate-50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${getSeverityColor(event.severity)}`}>
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className={`prose prose-sm max-w-none ${
              message.type === MESSAGE_TYPES.USER ? 'prose-invert' : ''
            }`}>
              <ReactMarkdown
                components={{
                  // Enhanced custom renderers
                  pre: ({ children }) => (
                    <pre className={`${
                      message.type === MESSAGE_TYPES.USER 
