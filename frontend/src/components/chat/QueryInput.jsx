...
const QueryInput = ({ onSendMessage, disabled = false, placeholder = "Ask me anything..." }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
          ...
          <div className="flex items-end p-4 gap-3">
            <div className="flex-1 relative">
              <textarea
                ...
                disabled={disabled}
                rows={1}
                className="w-full resize-none bg-transparent border-none outline-none text-slate-800 placeholder-slate-400 text-base leading-relaxed pr-24"
              ...
              <div className="absolute right-0 bottom-0 flex items-center gap-1">
                  ...
                  className={`p-2.5 rounded-xl transition-all duration-200 ${
                    isRecording 
                      ...
                      : disabled 
                        ...
                        : 'text-slate-400 hover:text-blue-600 hover:bg-blue-50 hover:shadow-md hover:scale-105 active:scale-95'
                  }`}
                ...
                <button
                  type="button"
                  disabled={true}
                  className="p-2.5 rounded-xl text-slate-300 cursor-not-allowed relative group"
              ...
              {!isRecording && query.length === 0 && isFocused && (
                <div className="flex items-center gap-4 text-xs text-slate-500">
                  <div className="flex items-center gap-1">
                    <div className="px-1.5 py-0.5 bg-slate-100 rounded text-xs font-mono">Enter</div>
                    <span>to send</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <div className="px-1.5 py-0.5 bg-slate-100 rounded text-xs font-mono">Shift+Enter</div>
                    <span>new line</span>
                  </div>
                </div>
              )}
      ...
      {disabled && (
        <div className="mt-3 flex items-center justify-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-slate-600 text-sm">
